...
export default function AppLayout({ children }: AppLayoutProps) {
  ...
  const navClassName = (active: boolean) =>
    active
      ? 'flex-none rounded-[10px] px-[14px] py-2 text-sm font-semibold text-sea transition hover:bg-sea-soft'
      : 'flex-none rounded-[10px] px-[14px] py-2 text-sm font-semibold text-ink-mute transition hover:bg-sea-soft hover:text-sea';
      ...
      <header className="sticky top-0 z-50 border-b border-line bg-white/90 backdrop-blur-[10px] backdrop-saturate-[180%]">
        <div className="mx-auto flex h-[66px] max-w-[1200px] items-center justify-between gap-3 px-4 sm:gap-6 sm:px-7">
          <Link to="/" className="flex flex-none items-center gap-[9px] p-0">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-sea">
              <svg viewBox="0 0 64 40" width="18" height="12" fill="#fff" aria-hidden>
                ...
                <circle cx="18" cy="17" r="2.4" fill="#0F6E84" />
              </svg>
            </span>
          ...
          <Link
            to="/saved"
            className="ml-auto inline-flex h-10 flex-none items-center gap-1.5 rounded-[10px] border border-line bg-white px-[14px] text-[13px] font-semibold text-ink transition hover:border-sea hover:text-sea"
            aria-label="저장한 도감"
          >
            <Bookmark className="h-4 w-4 fill-none text-sea" aria-hidden />
