...
export default function CalendarPage() {
  ...
  return (
    <main className="mx-auto max-w-content px-4 pb-20 pt-9 sm:px-7">
      ...
      <div className="-mx-4 mb-8 overflow-x-auto px-4 sm:-mx-7 sm:px-7 lg:mx-0 lg:overflow-visible lg:px-0">
        <div className="flex min-w-max gap-2 lg:grid lg:min-w-0 lg:grid-cols-12 lg:gap-1.75">
          {months.map((month) => {
              ...
              <button
                ...
                className={
                  active
                    ...
                    : 'flex h-[58px] w-[74px] flex-none flex-col items-center justify-center rounded-[11px] border border-line bg-surface px-2 text-ink transition hover:border-sea hover:text-sea lg:w-auto'
                }
                aria-pressed={active}
              >
                <span className="text-14 font-bold leading-[1.15]">{month}월</span>
                <span className={active ? 'mt-1 text-11 font-semibold leading-none text-white/85' : 'mt-1 text-11 font-semibold leading-none text-ink-mute/70'}>
      ...
      <h2 className="mb-5 text-[22px] font-bold tracking-[-0.02em] text-ink">
        {selectedMonth}월 제철 <span className="text-17 font-medium text-ink-mute/70">· {isMonthLoading || isMonthError ? '-' : monthFishes.length}종</span>
      </h2>
