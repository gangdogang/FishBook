  ...
  return (
    <main className="mx-auto max-w-[1200px] px-4 pb-20 pt-8 sm:px-7">
      <div className="mb-[22px] flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="mb-1.5 text-[13px] font-semibold tracking-[0.02em] text-brand-700">SAVED FISH</p>
          <h1 className="m-0 text-2xl font-bold tracking-[-0.025em] text-ink">
            저장한 도감 <span className="text-lg font-medium text-faint">{bookmarkCount}종</span>
          </h1>
        </div>
        <Link to="/search" className="text-sm font-semibold text-brand-700 transition hover:text-brand-500">
          전체 도감 둘러보기
        </Link>
      </div>
...
function EmptyState() {
  return (
    <div className="rounded-card border border-dashed border-[#E2E7E9] px-5 py-[72px] text-center">
      ...
      <p className="mb-5 text-[14.5px] leading-[1.5] text-muted">도감에서 마음에 드는 생선을 저장해두면 여기에서 다시 볼 수 있어요.</p>
      <Link
        to="/search"
        className="inline-flex rounded-[10px] border border-brand-500 bg-white px-[22px] py-[11px] text-sm font-semibold text-brand-700 transition hover:bg-brand-50"
      >
        도감 둘러보기
      </Link>
    </div>
