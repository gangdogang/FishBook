...
export default function HomePage() {
  ...
  const { data: fishes = [], isLoading, isError } = useFishList({ season, taste, sort: 'popular' });
  const {
    data: featuredFishes = [],
  ...
  return (
    <main className="bg-white">
      <section className="border-b border-[#EEF1F2] bg-gradient-to-b from-[#F4F9F8] to-white">
        <div className="mx-auto max-w-[1200px] px-4 py-10 text-center sm:px-7 sm:pb-9">
          <div className="mb-[18px] inline-flex items-center gap-[7px] rounded-full bg-brand-50 px-[13px] py-1.5 text-[13px] font-semibold text-brand-700">
          ...
          <p className="mx-auto max-w-[480px] text-[16.5px] leading-[1.55] text-muted">
            제철·맛·가격으로 둘러보는 회 도감. 횟집 가기 전에 미리 만나보세요.
          </p>
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
            <span className="text-[13px] text-faint">인기 검색</span>
            {popularTags.map((tag) => (
              <button
                key={tag}
                type="button"
                onClick={() => goSearch(tag)}
                className="rounded-full border border-line bg-white px-3 py-[5px] text-[13px] text-ink transition hover:border-brand-500 hover:text-brand-700"
              >
                {tag}
              </button>
            ))}
      ...
      <section className="mx-auto max-w-[1200px] px-4 pb-2 pt-11 sm:px-7">
        <div className="mb-5 flex items-end justify-between gap-4">
          <div>
            <div className="mb-1.5 text-[13px] font-semibold tracking-[0.02em] text-brand-700">EDITOR'S PICK</div>
            <h2 className="m-0 text-[26px] font-bold tracking-[-0.025em] text-ink">지금 가장 사랑받는 회</h2>
          </div>
          <Link to="/search" className="flex flex-none items-center gap-1 text-sm text-muted transition hover:text-brand-700">
            전체 보기
      ...
      <section className="mx-auto max-w-[1200px] px-4 pb-20 pt-12 sm:px-7">
        <div className="mb-[22px] flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <h2 className="m-0 text-2xl font-bold tracking-[-0.025em] text-ink">
            전체 도감 <span className="text-lg font-medium text-faint">{isLoading ? '' : `${fishes.length}종`}</span>
          </h2>
          <FilterChips season={season} taste={taste} onSeasonChange={setSeason} onTasteChange={setTaste} />
        </div>
...
function FeaturedFishCard({ fish }: { fish: FishSummary }) {
  ...
  return (
    <Link
      to={`/fish/${fish.id}`}
      className="group block overflow-hidden rounded-card border border-line bg-white transition duration-150 hover:-translate-y-[3px] hover:border-brand-500 hover:shadow-[0_14px_34px_rgba(15,148,136,0.10)]"
    >
      <div className="relative flex aspect-[16/10] items-center justify-center bg-[#E6F4F2]">
        {fish.imageUrl ? (
          <img src={fish.imageUrl} alt={fish.name} className="h-full w-full object-cover" />
        ) : (
          <svg viewBox="0 0 64 40" width="110" height="69" fill="none" stroke="rgba(15,148,136,0.3)" strokeWidth="1.5" aria-hidden>
        ...
        <span className="absolute left-3.5 top-3.5 flex items-center gap-[5px] rounded-full bg-ink px-[11px] py-[5px] text-xs font-semibold text-white">
          <span className="text-accent">★</span> {fish.avgRating.toFixed(1)}
        </span>
        <button
          type="button"
          onClick={handleBookmarkClick}
          className="absolute right-3 top-3 inline-flex h-[34px] w-[34px] items-center justify-center rounded-full border-0 bg-white/95 text-faint shadow-sm transition hover:text-brand-700"
      ...
      <div className="px-5 pb-5 pt-[18px]">
        <div className="mb-[7px] flex min-w-0 items-baseline gap-[9px]">
          ...
          <span className="ml-auto flex-none text-[15px] font-semibold text-brand-700">{formatPriceLevel(fish.priceLevel)}</span>
        </div>
        {fish.description ? <p className="mb-3.5 line-clamp-2 text-sm leading-[1.5] text-muted">{fish.description}</p> : null}
        <div className="flex flex-wrap items-center gap-[7px]">
          <span className="rounded-full bg-brand-50 px-[11px] py-[5px] text-[12.5px] font-semibold text-brand-700">
            {formatMonths(fish.seasonMonths)} 제철
          </span>
          {fish.tasteTags.slice(0, 2).map((tag) => (
            <span key={tag} className="rounded-full border border-[#EDF0F2] bg-[#F7F9FA] px-[11px] py-[5px] text-[12.5px] text-muted">
              {tag}
            </span>
          ))}
        </div>
